    ...
    const fetchAdmins = useCallback(async () => {
        try {
            ...
            toast({
                variant: 'destructive',
                title: 'Hata',
                description: `Yöneticiler yüklenirken hata: ${error.message || 'Bilinmeyen hata'}`
            })
        }
    }, [restaurant.id, toast])
                        ...
                        <div className="flex items-center gap-2">
                            {BRAND_COLORS.map((color) => (
                                <button
                                    ...
                                    className={`w-10 h-10 rounded-lg transition-all ${formData.brand_color === color
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {PERSONALITY_OPTIONS.map((option) => (
                                <button
                                    ...
                                    className={`p-4 rounded-lg border-2 text-left transition-all ${formData.ai_personality === option.value
